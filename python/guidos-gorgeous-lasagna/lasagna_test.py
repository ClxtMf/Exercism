...
# Para este primeiro exercício, é muito importante deixar claro como estamos importando nomes para testes.
# Para esse fim, estamos colocando um try/catch nas importações e lançando mensagens específicas para ajudar os alunos
# decodifique o que eles precisam para criar e nomear suas constantes e funções de uma maneira específica.
try:
...
# Aqui, estamos separando os erros de importação constantes dos erros de importação do nome da função
...
# Aqui começam os casos de teste formais para o exercício.
class LasagnaTest(unittest.TestCase):
    ...
    def test_docstrings_were_written(self):
        """Validate function.__doc__ existe para cada função.
        Verifique o dicionário de atributos de cada função listada
        pela presença de uma chave __doc__.

        :return: inesperadamente Nenhum erro quando a chave __doc__ está faltando.
        """
        ...
        for variant, function in enumerate(functions, start=1):
            with self.subTest(f'variation #{variant}', function=function):
                ...
                # Verifique se a chave __doc__ está preenchida para a função.
